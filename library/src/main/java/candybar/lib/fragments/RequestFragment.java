...
public class RequestFragment extends Fragment implements View.OnClickListener {
    ...
    private final AtomicBoolean hasDetailsLoaded = new AtomicBoolean(false);
    ...
    public void onResume() {
        super.onResume();
        // Show request consent dialog if not accepted
    ...
    public void prepareRequest() {
        ...
        // Check free request limit
        int freeRequestLimit = getResources().getInteger(R.integer.icon_request_limit);
        int usedFreeRequests = Preferences.get(requireActivity()).getRegularRequestCount();
        int remainingFreeRequests = freeRequestLimit - usedFreeRequests;

        if (!Preferences.get(requireActivity()).isPremiumRequest() && 
            sSelectedRequests != null && 
            sSelectedRequests.size() > remainingFreeRequests) {
            RequestHelper.showFreeRequestLimitDialog(requireActivity(), remainingFreeRequests);
            return;
        }

        // If request proceeds, increment the counter
        if (!Preferences.get(requireActivity()).isPremiumRequest() && sSelectedRequests != null) {
            Preferences.get(requireActivity())
                .setRegularRequestCount(usedFreeRequests + sSelectedRequests.size());
        }
    ...
    private class RequestLoader extends AsyncTaskBase {
        ...
        private void loadPremiumRequest() {
            if (Preferences.get(requireActivity()).isPremiumRequest()) {
                InAppBillingClient.get(requireActivity()).getProcessor().queryPurchases(
    ...
    private void loadApps() {
        // ... existing loadApps code ...

        // Add this after loading apps
        int freeRequestLimit = getResources().getInteger(R.integer.icon_request_limit);
        int usedFreeRequests = Preferences.get(requireActivity()).getRegularRequestCount();
        
        // Check if we've hit the free request limit
        if (usedFreeRequests >= freeRequestLimit) {
            for (Request request : mRequests) {
                if (!request.isRequested()) {
                    request.setAvailableForRequest(false);
                    request.setInfoText(getString(R.string.request_limit_reached));
                }
            }
        }
    }
