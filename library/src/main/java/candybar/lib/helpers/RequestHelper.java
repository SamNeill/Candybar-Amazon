...
public class RequestHelper {
    ...
    public static void showFreeRequestLimitDialog(@NonNull Context context, int remainingRequests) {
        new MaterialDialog.Builder(context)
            .typeface(TypefaceHelper.getMedium(context), TypefaceHelper.getRegular(context))
            .title(R.string.request_limit_title)
            .content(context.getString(R.string.request_limit_content, remainingRequests))
            .positiveText(R.string.close)
            .negativeText(R.string.premium_request)
            .onNegative((dialog, which) -> {
                if (context instanceof CandyBarMainActivity) {
                    ((CandyBarMainActivity) context).onBuyPremiumRequest();
                }
            })
            .show();
    }
